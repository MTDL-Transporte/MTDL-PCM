...
def setup_exact_500h():
    print("=== CONFIGURANDO EXATAMENTE 500H TRABALHADAS ===")
    
    # Conectar ao banco de dados
    conn = sqlite3.connect('mtdl_pcm.db')
    cursor = conn.cursor()
    
    # Para ter exatamente 500h trabalhadas com horímetro atual de 310h:
    # initial_horimeter = 310 - 500 = -190h
    target_hours_worked = 500.0
    ...
    print(f"  500h >= {next_maintenance_250}h? {target_hours_worked >= next_maintenance_250}")
    ...
    print(f"  500h >= {next_maintenance_500}h? {target_hours_worked >= next_maintenance_500}")
    ...
    # Teste com 500.1h para garantir que ultrapasse
    print(f"\n🔍 Teste conceitual com 500.1h:")
    test_hours = 500.1
    cycles_500_test = int(test_hours // 500)
    next_maintenance_500_test = (cycles_500_test + 1) * 500
    print(f"  Ciclos completados: {cycles_500_test}")
    print(f"  Próxima manutenção: {next_maintenance_500_test}h")
    print(f"  500.1h >= {next_maintenance_500_test}h? {test_hours >= next_maintenance_500_test}")
